# -*- coding: utf-8 -*-
# This script originates from the HistomicsTK repository
# Available at: https://github.com/DigitalSlideArchive/HistomicsTK

import numpy as np

from ._compute_marginal_glcm_probs_cython import _compute_marginal_glcm_probs_cython
from .graycomatrixext import _default_num_levels, _default_offsets, graycomatrixext
...
def compute_haralick_features(
    ...
    for i in range(numLabels):
        ...
        ldata = pd.DataFrame(
            np.zeros((num_offsets, len(feature_list))), columns=feature_list
        )
        ...
        for r in range(num_offsets):
            ...
            # computes contrast
            ldata.at[r, "Haralick.Contrast"] = np.dot(np.square(n_Minus), pxMinusy)
            ...
            ldata.at[r, "Haralick.Correlation"] = (
                np.dot(np.ravel(xy), nGLCMr) - np.square(meanx)
            ) / variance
            ...
            ldata.at[r, "Haralick.IDM"] = np.dot(np.ravel(xy_IDM), nGLCMr)

            # computes sum average
            ldata.at[r, "Haralick.SumAverage"] = np.dot(n_Plus, pxPlusy)

            # computes sum variance
            # [1] uses sum entropy, but we use sum average
            ldata.at[r, "Haralick.SumVariance"] = np.dot(
                np.square(n_Plus), pxPlusy
            ) - np.square(ldata.at[r, "Haralick.SumAverage"])

            # computes sum entropy
            ldata.at[r, "Haralick.SumEntropy"] = -np.dot(pxPlusy, np.log2(pxPlusy + e))

            # computes entropy
            ldata.at[r, "Haralick.Entropy"] = -np.dot(nGLCMr, np.log2(nGLCMr + e))

            # computes variance px-y
            ldata.at[r, "Haralick.DifferenceVariance"] = np.var(pxMinusy)

            # computes difference entropy px-y
            ldata.at[r, "Haralick.DifferenceEntropy"] = -np.dot(
                pxMinusy, np.log2(pxMinusy + e)
            )
            ...
            # computes information measures of correlation
            ldata.at[r, "Haralick.IMC2"] = np.sqrt(1 - np.exp(-2.0 * (HXY2 - HXY)))
