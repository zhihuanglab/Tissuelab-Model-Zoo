...
class DetectionCellPostProcessor:
    """CPU-only version of DetectionCellPostProcessorCupy"""
    def __init__(
        self,
        ...
        Args:
            ...
            nr_types (int):  Number of cell types, including background (background = 0)
        ...
        Raises:
            NotImplementedError: Unknown resolution
        """
        self.wsi = wsi
    ...
    def _prepare_pred_maps(self, predictions_: dict) -> np.ndarray:
        """Prepares the prediction maps for post-processing using numpy instead of cupy"""
        predictions = predictions_.copy()
        predictions["nuclei_type_map"] = predictions["nuclei_type_map"].numpy()
        predictions["nuclei_binary_map"] = predictions["nuclei_binary_map"].numpy()
        predictions["hv_map"] = predictions["hv_map"].numpy()
    ...
    def _stack_pred_maps(
        ...
        """Creates the prediction map using numpy operations"""
    ...
    def _proc_np_hv(
        ...
        """Process Nuclei Prediction with XY Coordinate Map using CPU operations"""
        ...
        labeled_array, num_features = label(blb)
        blb = remove_small_objects(labeled_array, min_size=10)
        blb[blb > 0] = 1
...
class BatchPoolingActor:
    def __init__(
        ...
        The calculation is done with the help of the `DetectionCellPostProcessor` class.
        This actor acts as a coordinator for the postprocessing of one batch and a wrapper for the `DetectionCellPostProcessor` class.

        Args:
            detection_cell_postprocessor (DetectionCellPostProcessor): Instance of the `DetectionCellPostProcessor` class
