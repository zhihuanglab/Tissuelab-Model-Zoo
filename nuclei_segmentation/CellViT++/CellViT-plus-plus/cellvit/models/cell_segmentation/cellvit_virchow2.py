...
class CellViTVirchow2(CellViT):
    """CellViT with ViT-Virchow2 backbone settings

    Virchow Links:
        Paper: https://doi.org/10.48550/arXiv.2408.00738
    ...
    def forward(self, x: torch.Tensor, retrieve_tokens: bool = False) -> dict:
        ...
        z4 = z4[:, 5:, :].transpose(-1, -2).view(-1, self.embed_dim, *patch_dim)
        z3 = z3[:, 5:, :].transpose(-1, -2).view(-1, self.embed_dim, *patch_dim)
        z2 = z2[:, 5:, :].transpose(-1, -2).view(-1, self.embed_dim, *patch_dim)
        z1 = z1[:, 5:, :].transpose(-1, -2).view(-1, self.embed_dim, *patch_dim)
