...
class BaseTrainer:
    ...
    Methods:
        ...
        fit(epochs: int, train_dataloader: DataLoader, val_dataloader: DataLoader, metric_init: dict = None, eval_every: int = 1, **kwargs):
            Fitting function to start training and validation of the trainer
        save_checkpoint(epoch: int, checkpoint_name: str):
            Save checkpoint
        resume_checkpoint(checkpoint: dict, just_load_model: bool = False):
            Resume checkpoint from a dictionary
    """
    ...
    def resume_checkpoint(self, checkpoint: dict, just_load_model: bool = False):
        """Resume checkpoint from a dictionary

        Args:
            checkpoint (dict): Checkpoint dictionary
            just_load_model (bool, optional): If just model and not all states (e.g. optimizer) should be loaded.
                Defaults to False.
        """
        self.logger.info("Loading checkpoint")
        self.logger.info("Loading Model")
        self.logger.info(self.model.load_state_dict(checkpoint["model_state_dict"]))
        if not just_load_model:
            ...
            self.logger.info(
                f"Just loaded model from state dict, all other states are used from scratch (optimizer, scheduler, epochs)"
            )
