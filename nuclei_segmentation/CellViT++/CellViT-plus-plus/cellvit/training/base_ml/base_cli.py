...
class ExperimentBaseParser:
    ...
    def __init__(self) -> None:
        ...
        # Adding just_load_model flag
        parser.add_argument(
            "--just_load_model",
            action="store_true",
            help="If specified with --checkpoint, only the model will be loaded without continuing training.",
        )
        self.parser = parser
    ...
    def parse_arguments(self) -> Tuple[Union[BaseModel, dict]]:
        ...
        # validate
        if opt.just_load_model and not opt.checkpoint:
            parser.error("--just_load_model requires --checkpoint to be set.")
        ...
        if "checkpoint" in opt_dict:
            ...
            if "just_load_model" in opt_dict:
                yaml_config_dict["just_load_model"] = opt_dict["just_load_model"]
