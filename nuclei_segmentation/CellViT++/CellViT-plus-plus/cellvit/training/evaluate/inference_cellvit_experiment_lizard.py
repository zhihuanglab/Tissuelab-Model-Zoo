...
from cellvit.training.datasets.lizard import LizardGraphDataset, LizardHistomicsDataset
...
def process_entry(pred, prob, inform, cell_dict, centroid_dict):
    ...
    if (
        ...
        cell_dict[image_name][cell_idx]["type_prob"] = float(prob[int(pred)])
...
class CellViTInfExpLizard(CellViTClassifierInferenceExperiment):
    """CellViT Inference Experiment for Lizard Dataset
    ...
    def _load_dataset(
        ...
        """
        dataset = LizardGraphDataset(
            dataset_path=self.dataset_path, split=split, network_name=network_name
        )
...
class CellViTInfExpLizardHistomics(CellViTInfExpLizard):
    def __init__(
        self,
        logdir: Union[Path, str],
        dataset_path: Union[Path, str],
...
class CellViTInfExpLizardParser:
    def __init__(self) -> None:
        ...
        parser.add_argument(
            "--use_histomics",
            action="store_true",
            help="Flag to indicate whether to use histomics features",
        )
        parser.add_argument(
            "--norm_path",
            type=str,
...
if __name__ == "__main__":
    ...
    if not configuration["use_histomics"]:
        experiment_inferer = CellViTInfExpLizard(
