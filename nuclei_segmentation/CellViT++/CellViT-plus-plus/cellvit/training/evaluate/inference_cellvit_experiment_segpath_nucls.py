...
class CellViTInfExpSegPathNuCLS(CellViTClassifierInferenceExperiment):
    # TODO: Docstring
    def __init__(
        self,
    ...
    def _calculate_pipeline_scores(self, cell_dict: dict) -> Tuple[dict, dict, dict]:
        ...
        for image_idx, (image_name, cells) in tqdm.tqdm(
            ...
            cell_annot = [
                (int(row["x"]), int(row["y"]), row["main_classification"])
            ...
            types_gt = [
                1 if self.inference_dataset.inverse_label_map[self.label] == t else 0
                for t in types_gt
            ]
        ...
        cls_idx_to_name = {0: "other", 1: self.label}
    ...
    def run_inference(self):
        ...
        with torch.no_grad():
            for _, (images, cell_gt_batch, types_batch, image_names) in tqdm.tqdm(
                ...
                # change the type to the selected label
                types_batch_refactored = []
                for img_types in types_batch:
                    types_batch_refactored.append(
                        [
                            1
                            if self.inference_dataset.inverse_label_map[self.label] == t
                            else 0
                            for t in img_types
                        ]
                    )
                (
                    ...
                    types_batch=types_batch_refactored,
        ...
        cleaned_inference_results = self._get_classifier_result(
            extracted_cells_cleaned, self.threshold
        )
        ...
        inference_results = self._get_classifier_result(
            extracted_cells, threshold=self.threshold
        )
        ...
        json_label_map = {0: "other", 1: self.label}
...
class CellViTInfParserSegPathNuCLS:
    def __init__(self) -> None:
        ...
        parser.add_argument(
            ...
            default="/home/jovyan/cellvit-data/cellvit/logs_paper/Head-Evaluation/NuCLS/Clean-Dataset/plasma/SAM-H/Fold-2",
        )
        parser.add_argument(
            "--dataset_path",
            type=str,
            help="Path to the NuCLS dataset",
            default="/home/jovyan/cellvit-data/NuCLS/corrected-dataset-ouf-of-bounds",
        )
        parser.add_argument(
            "--label",
            type=str,
        ...
        parser.add_argument(
            ...
            default="/home/jovyan/cellvit-data/cellvit/checkpoints/CellViT-SAM-H-x40-AMP.pth",
        )
        parser.add_argument(
            "--normalize_stains",
            action="store_true",
...
if __name__ == "__main__":
    configuration_parser = CellViTInfParserSegPathNuCLS()
    configuration = configuration_parser.parse_arguments()

    experiment_inferer = CellViTInfExpSegPathNuCLS(
