...
class CellViTInfExpPanopTILS(CellViTClassifierInferenceExperiment):
    """Inference Experiment for CellViT with a Classifier Head on PanopTILs Data
    ...
    Attributes:
        ...
        fov (str): Dict with field of view for each image
    """
    ...
    def __init__(
        ...
        # load FOV
        self.fov = pd.read_csv(self.dataset_path / "test" / "FOV.csv", index_col=0)

    def _load_dataset(self, transforms: Callable, normalize_stains: bool) -> Dataset:
        """Load PanopTILs Dataset (Used split: Test)
        ...
        Returns:
            Dataset: PanopTILs Dataset
        """
        dataset = PanoptilsDataset(
    ...
    def _clean_fov(self, cell_dict: List[dict], image_names: List[str]) -> List[dict]:
        """Clean cell dict such that only cells in FOV are considered

        Args:
            cell_dict (List[dict]): List of cell dictionarys
            image_names (List[str]): List of image names

        Returns:
            List[dict]: Cell dictionary list (FOV)
        """
        cleaned_cell_dicts = []
        for cell_pred_dict, img_name in zip(cell_dict, image_names):
            cleaned_cells = []
            img_name = f"{img_name}.png"
            img_fov = self.fov[self.fov["roiname"] == img_name]
            for cell_idx, cell in cell_pred_dict.items():
                ...
                if (
                    x >= int(img_fov["left"])
                    and x <= int(img_fov["right"])
                    and y >= int(img_fov["top"])
                    and y <= int(img_fov["bottom"])
                ):
                    cleaned_cells.append(cell)
            cleaned_cells = {i + 1: x for i, x in enumerate(cleaned_cells)}
            cleaned_cell_dicts.append(cleaned_cells)
        return cleaned_cell_dicts
    ...
    def _get_cellvit_result(
        ...
        # transform predictions, clean predictions such that only predictions only in FOV are visible and create tokens
        predictions = self._apply_softmax_reorder(predictions)
        _, cell_pred_dict = postprocessor.post_process_batch(predictions)
        cell_pred_dict = self._clean_fov(cell_pred_dict, image_names)
    ...
    def _calculate_pipeline_scores(self, cell_dict: dict) -> Tuple[dict, dict, dict]:
        ...
        # like ocelot?
        annot_path = self.dataset_path / "test" / "annotations"
    ...
    def run_inference(self):
        """Run Inference on Test Dataset for PanopTILs data"""
...
class CellViTInfExpPanopTILSParser:
    def __init__(self) -> None:
        parser = argparse.ArgumentParser(
            formatter_class=argparse.ArgumentDefaultsHelpFormatter,
            description="Perform CellViT-Classifier inference for PanopTILS",
        )
        parser.add_argument(
            "--logdir",
            type=str,
        ...
        parser.add_argument(
            "--dataset_path",
            type=str,
            help="Path to the PanopTILs dataset",
            default="/home/jovyan/cellvit-data/panoptils",
        )
        parser.add_argument(
            "--cellvit_path",
            type=str,
            help="Path to the Cellvit model",
            default="/home/jovyan/cellvit-data/cellvit/checkpoints/CellViT-UNI-x40-AMP.pth",
        )
        parser.add_argument(
            "--normalize_stains",
            action="store_true",
...
if __name__ == "__main__":
    configuration_parser = CellViTInfExpPanopTILSParser()
    configuration = configuration_parser.parse_arguments()

    experiment_inferer = CellViTInfExpPanopTILS(
