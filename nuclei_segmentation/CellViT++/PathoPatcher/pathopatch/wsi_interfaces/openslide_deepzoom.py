# -*- coding: utf-8 -*-
# DeepZoom implementation adapted from OpenSlide Python
#
...
if TYPE_CHECKING:
    ...
    from tiffslide import TiffSlide  # Update import

class DeepZoomGeneratorTS:  # Renamed from DeepZoomGeneratorOS
    """Generates Deep Zoom tiles and metadata for TiffSlide."""

    def __init__(
        self, 
        slide: TiffSlide,  # Updated type hint
        tile_size: int = 254,
        overlap: int = 1,
        limit_bounds: bool = False,
        **kwargs
    ):
        """Create a DeepZoomGenerator wrapping a TiffSlide object.

        Args:
            slide: a TiffSlide object
        ...
        """
        self._ts = slide  # Renamed from _osr
        self._z_t_downsample = tile_size
        self._z_overlap = overlap
        self._limit_bounds = limit_bounds

        # Initialize dimensions
        self._l_dimensions = slide.level_dimensions
        self._l0_offset = (0, 0)
        ...
        # Set number of Deep Zoom levels
        ...
        # Get preferred slide levels
        self._slide_from_dz_level = tuple(
            self._ts.get_best_level_for_downsample(d) for d in l0_z_downsamples
        )

        # Calculate piecewise downsamples
        self._l0_l_downsamples = self._ts.level_downsamples
    ...
    def get_tile(self, level: int, address: tuple[int, int]) -> Image.Image:
        ...
        tile = self._ts.read_region(*args)
        profile = tile.info.get('icc_profile')

        # Apply on solid background
        bg = Image.new('RGB', tile.size, self._bg_color)
        if 'A' in tile.mode:  # Check if image has alpha channel
            # Use alpha channel as mask
            mask = tile.split()[-1]
            tile = Image.composite(tile.convert('RGB'), bg, mask)
        else:
