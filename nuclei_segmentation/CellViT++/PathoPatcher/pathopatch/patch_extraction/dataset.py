...
from pathopatch.wsi_interfaces.openslide_deepzoom import DeepZoomGeneratorTS
...
class LivePatchWSIDataset(Dataset):
    def __init__(
        ...
        Attributes:
            slide_openslide (TiffSlide): OpenSlide object for the slide
            image_loader (Union[TiffSlide, Any]): Image loader for the slide, method for loading the slide
            slide (Union[TiffSlide, Any]): Extraction object for the slide (OpenSlide, CuCIM, wsiDicomizer), instance of image_loader
            wsi_metadata (dict): Metadata of the WSI
            deepzoomgenerator (Union[DeepZoomGeneratorTS, Any]): Class for tile extraction, deepzoom-interface
            tile_extractor (Union[DeepZoomGeneratorTS, Any]): Instance of self.deepzoomgenerator
        ...
        """
        # slide specific
        self.slide_openslide: TiffSlide
        self.image_loader: Union[TiffSlide, Any]
        self.slide: Union[TiffSlide, Any]
        self.wsi_metadata: dict
        self.deepzoomgenerator: Union[
            DeepZoomGeneratorTS, Any
    ...
    def _set_hardware(self) -> None:
        """Either load CuCIM (GPU-accelerated) or TiffSlide"""
        ...
        if wsi_file.is_dir():
            if len(list(wsi_file.glob("*.dcm"))) != 0:
                ...
                try:
                    ...
                    TiffSlide(str(dcm_files[0][0]))
                    wsi_file = dcm_files[0][0]
                    self.image_loader = TiffSlide
                    self.deepzoomgenerator = DeepZoomGeneratorTS
                    self.slide_metadata_loader = TiffSlide
                except:
                    try:
                        DicomSlide(wsi_file)
            ...
            if module_exists("cucim", error="ignore"):
                ...
                self.logger.debug("Using TiffSlide")
                self.deepzoomgenerator = DeepZoomGeneratorTS
                self.image_loader = TiffSlide
                self.slide_metadata_loader = TiffSlide
    ...
    def _prepare_slide(
        ...
        elif "tiffslide.mpp-x" in self.slide_openslide.properties:
            slide_mpp = float(self.slide_openslide.properties["tiffslide.mpp-x"])
        ...
        elif "tiffslide.objective-power" in self.slide_openslide.properties:
            slide_mag = float(
                self.slide_openslide.properties.get("tiffslide.objective-power")
            )
        else:
            raise NotImplementedError(
                "MPP must be defined either by metadata or by config file!"
