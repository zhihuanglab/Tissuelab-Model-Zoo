...
Verify (in the same conda env as the task node, not `base`):
```python
import pyvips
print(pyvips.version(0), pyvips.version(1), pyvips.version(2))
```

Linux/Windows NVIDIA: the `pip install -r` above already pulls CUDA 12.6 PyTorch (`cu126`) and `tensorflow[and-cuda]`. macOS gets CPU/MPS wheels.
