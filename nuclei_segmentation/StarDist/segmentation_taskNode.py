...
def parse_args():
    ...
    parser.add_argument('--stardist_pretrain', default='2D_versatile_fluo', type=str,
...
def run_segmentation(args):
    ...
    try:
        ...
        if centroids is not None and len(centroids) > 0: # Ensure centroids exist and are not empty
            ...
            if not have_cached_embedding and NODE_NAME in zf and 'embedding' in zf[NODE_NAME]:
                _emb_model = dict(zf[NODE_NAME]['embedding'].attrs).get('model')
                if _emb_model:
                    raise RuntimeError(
                        f"Existing '{NODE_NAME}/embedding' was generated by '{_emb_model}' but its row count "
                        f"({zf[NODE_NAME]['embedding'].shape[0]}) no longer matches the segmentation "
                        f"({len(centroids)} cells). Refusing to overwrite it with PLIP embeddings - "
                        f"re-run the {_emb_model} extraction for this slide instead.")
...
def read_node(data: Dict[str, Any]):
    ...
    if ARGS is None:
        ARGS = argparse.Namespace(
            ...
            stardist_pretrain="2D_versatile_fluo",
            isIHC=False,
