...
_NORM_MEAN_TENSOR = None  # Will be initialized on first use
...
def _normalize_norm_gpu(tensor, device):
    ...
    if _NORM_MEAN_TENSOR is None or _NORM_MEAN_TENSOR.device != device:
        _NORM_MEAN_TENSOR = torch.tensor(_NORM_MEAN_UINT8, device=device, dtype=torch.float32).view(1, 3, 1, 1)
...
def _preprocess_norm_gpu(images, device, target_size=224, perf_stats=None):
    ...
    # Step 3: Normalize with ImageNet statistics (GPU-accelerated)
    normalized_batch = _normalize_norm_gpu(batch_tensor, device)
...
def _preprocess_norm_gpu_with_region_crop(region_images, crop_coords_list, device, target_size=224, extraction_size=None, perf_stats=None):
    """
    ...
    for region_img, crop_coords in zip(region_images, crop_coords_list):
        ...
        for local_x, local_y, ext_size in crop_coords:
            ...
            # Resize to target_size on GPU (matching _preprocess_norm_gpu behavior)
            # This matches the CPU preprocessing flow: resize shortest edge, then center crop
            h, w = patch.shape[2], patch.shape[3]
            shortest_edge = min(h, w)
            
            # Calculate scale factor based on shortest edge (matching _preprocess_norm_gpu)
            scale = target_size / shortest_edge
            new_h = int(np.floor(h * scale))
            new_w = int(np.floor(w * scale))
            ...
            # Center crop on GPU (matching _preprocess_norm_gpu)
            if new_h != target_size or new_w != target_size:
                top = (new_h - target_size) // 2
                left = (new_w - target_size) // 2
    ...
    # Normalize with ImageNet statistics (GPU-accelerated)
    normalized_batch = _normalize_norm_gpu(batch_tensor, device)
                ...
                try:
                    ...
                    if stacked.shape[1:] == (h, w, 3):
                        ...
                        if return_uint8:
                            ...
                            batch_array[:, 1] -= _NORM_MEAN_TIMES_INV_STD_UINT8[1]
                            np.multiply(float_batch[:, :, :, 2], _NORM_INV_STD_UINT8[2], out=batch_array[:, 2])
                            batch_array[:, 2] -= _NORM_MEAN_TIMES_INV_STD_UINT8[2]
                        ...
                        batch_array[:, 0] -= _NORM_MEAN_TIMES_INV_STD_UINT8[0]
                        np.multiply(float_batch, _NORM_INV_STD_UINT8[1], out=batch_array[:, 1])
                        batch_array[:, 1] -= _NORM_MEAN_TIMES_INV_STD_UINT8[1]
                        np.multiply(float_batch, _NORM_INV_STD_UINT8[2], out=batch_array[:, 2])
                        batch_array[:, 2] -= _NORM_MEAN_TIMES_INV_STD_UINT8[2]
                        normalize_time = time.time() - normalize_start
                        if perf_stats is not None:
                            perf_stats['processor_astype_time'] += 0.0  # Merged into normalize_time
            ...
            for i, img in enumerate(images):
                ...
                try:
                    ...
                    if arr_uint8.shape == (h, w, 3):
                        ...
                        batch_array[i, 0] -= _NORM_MEAN_TIMES_INV_STD_UINT8[0]
                        np.multiply(float_img[:, :, 1], _NORM_INV_STD_UINT8[1], out=batch_array[i, 1])
                        batch_array[i, 1] -= _NORM_MEAN_TIMES_INV_STD_UINT8[1]
                        np.multiply(float_img[:, :, 2], _NORM_INV_STD_UINT8[2], out=batch_array[i, 2])
                        batch_array[i, 2] -= _NORM_MEAN_TIMES_INV_STD_UINT8[2]
                        normalize_time = time.time() - normalize_start
                        if perf_stats is not None:
                            perf_stats['processor_astype_time'] += 0.0  # Merged into normalize_time
                            perf_stats['processor_normalize_time'] += normalize_time
                        ...
                        batch_array[i, 0] -= _NORM_MEAN_TIMES_INV_STD_UINT8[0]
                        np.multiply(float_img, _NORM_INV_STD_UINT8[1], out=batch_array[i, 1])
                        batch_array[i, 1] -= _NORM_MEAN_TIMES_INV_STD_UINT8[1]
                        np.multiply(float_img, _NORM_INV_STD_UINT8[2], out=batch_array[i, 2])
                        batch_array[i, 2] -= _NORM_MEAN_TIMES_INV_STD_UINT8[2]
                        normalize_time = time.time() - normalize_start
                        if perf_stats is not None:
                            perf_stats['processor_astype_time'] += 0.0  # Merged into normalize_time
...
class NucleiEmbedding:
    ...
    def init_model(self):
        ...
        print(f"Cytoformer embedder ready (embed_dim={self.cyto.embed_dim})")
                ...
                if hasattr(torch, 'compile') and torch.cuda.is_available():
                    ...
                    if not triton_available:
                        ...
                        try:
                            ...
                            print("[OPTIMIZATION] Cytoformer encoder compilation completed (mode='default')")
                        except Exception as compile_error:
                            print(f"[OPTIMIZATION] torch.compile failed, running eager: {compile_error}")
                else:
            ...
            for cell_idx, cell_patches in enumerate(processed_batch):
                ...
                if isinstance(cell_patches, list):
                    ...
                    with torch.inference_mode():
                        ...
                        if len(all_cell_embeddings) == 0:
                            print(f"[DEBUG] Before fusion: {embeddings.shape} (should be [z, 1536])")
                        ...
                        if len(all_cell_embeddings) == 0:
                            print(f"[DEBUG] After fusion: {fused_embedding.shape} (should be [1, 1536])")
    ...
    def generate_embeddings(self, batch_size=None, num_workers=None, zarr_path=None, dataset_path='embeddings'):
        ...
        # Cytoformer has no transformers processor (self.processor is None): always
        # use the norm-on-tensor collate, which handles raw uint8 patches and works
        # on a CPU device too (embed_prenorm needs already-normalized input).
        use_gpu_preprocess = torch.cuda.is_available() or self.processor is None
        ...
        if use_gpu_preprocess:
            def collate_with_gpu_preprocess(batch):
                ...
                if use_region_crop:
                    ...
                    if len(region_images) > 0:
                        region_patches = _preprocess_norm_gpu_with_region_crop(
                            region_images,
                            crop_coords_list,
                            self.device,
                    ...
                    if len(regular_items) > 0:
                        regular_patches = _preprocess_norm_gpu(regular_items, self.device, target_size=dataset.patch_size, perf_stats=perf_stats)
                    else:
                ...
                if len(valid_items) > 0:
                    processed_tensors = _preprocess_norm_gpu(valid_items, self.device, target_size=dataset.patch_size, perf_stats=perf_stats)
        ...
        if is_zstack:
            ...
            for layer_idx in range(num_layers):
                ...
                layer_dset = parent.create_array(
                    ...
                    shape=(num_cells, self.cyto.embed_dim),
                    chunks=(min(1000, batch_size), self.cyto.embed_dim),
                    dtype=np.float16,
                    overwrite=True
                )
        ...
        embeddings_dset = parent.create_array(
            ds_name,
            shape=(len(dataset), self.cyto.embed_dim) if need_reorder else (0, self.cyto.embed_dim),
            chunks=(min(1000, batch_size), self.cyto.embed_dim),
            dtype=np.float16,
            overwrite=True
        )
            ...
            for batch in dataloader:
                ...
                if batch_valid:
                    ...
                    if need_reorder:
                        ...
                        embeddings_dset.resize((new_size, self.cyto.embed_dim))
