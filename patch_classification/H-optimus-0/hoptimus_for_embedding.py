...
class MUSK:
    """H-optimus-0 patch-embedding wrapper. Keeps the MUSK wrapper's public
    interface (``preprocess``, ``encode_tensors``, ``process_whole_wsi``) so the
    shared task-node plumbing works unchanged — only the model loading,
    transform, forward and output dim (1536) differ. The class name is kept as
    ``MUSK`` so the copied task-node code needs no symbol renames."""
    def __init__(self, model_path="hf-hub:bioptimus/H-optimus-0"):
        ...
        # H-optimus-0 preprocessing (values from the model card). Input 224x224.
    ...
    def _load_model(self, model_path: str):
        """Load H-optimus-0. Prefers a local checkpoint
        (``checkpoints/pytorch_model.bin``) so the gated 4.2 GB weights never
        re-download and no HF auth is needed at runtime — the weights load
        cleanly into the stock timm ``vit_giant_patch14_reg4_dinov2`` arch
        (verified: 0 missing / 0 unexpected keys). Falls back to pulling the
        gated model from HuggingFace (needs a token) only if no local file."""
        ...
        if os.path.isfile(local_bin):
            print(f"H-optimus-0: loading local checkpoint {local_bin}")
            ...
            return model
        # No local weights — pull the gated model from HF (needs auth).
        repo = (
            model_path if (model_path and model_path.startswith("hf-hub:"))
            else "hf-hub:bioptimus/H-optimus-0"
        )
        print(f"H-optimus-0: no local checkpoint, pulling {repo} from HuggingFace (gated)")
        return timm.create_model(repo, pretrained=True, init_values=1e-5, dynamic_img_size=False)
    ...
    def encode_tensors(self, batch_images_cpu: torch.Tensor, profile: bool = False, profile_acc: Optional[dict] = None):
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            # H-optimus-0's forward returns the class-token embedding directly,
            # shape (B, 1536). No with_head/out_norm/return_global kwargs.
            emb_gpu = self.model(images_gpu)
        if profile and self.device == 'cuda':
            torch.cuda.synchronize()
