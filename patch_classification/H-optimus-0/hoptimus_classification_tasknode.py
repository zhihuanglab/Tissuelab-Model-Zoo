...
# H-optimus-0 is vision-only: the model object is only used by the (disabled)
...
try:
    from hoptimus_for_embedding import MUSK  # noqa: F401 (kept for symbol compat)
except Exception:
    MUSK = None
...
def train_linear_classifier(
    ...
    if "Negative control" not in positive_annotations["class"].values.astype(str):
        ...
        if not hasattr(train_linear_classifier, '_negative_control_vectors'):
            ...
            neg_control_path = os.path.join(base_path, "negative_control_vectors_1536d.npy")
...
def run_ovr_classification(cell_embeddings, annotations, tissue_classes, tissue_colors, use_supervised):
    ...
    zero-shot over the classifier-less classes + Negative control (text similarity).
    Returns (predictions, prediction_probs, final_class_names, final_class_colors,
    method) or None to fall through to multiclass/zero-shot."""
...
def run_classification(args) -> Dict[str, Any]:
    ...
    try:
        ...
        meta_attrs = {
            'model': 'H-optimus-0-Classification',
            'classification_method': classification_method,
...
def main():
    ...
    parser.add_argument('--name', type=str, default='HOptimusClassification', help='node name')
