...
from hoptimus_for_embedding import MUSK
...
def parse_args():
    ...
    parser.add_argument('--name', type=str, default='HOptimusEmbedding', help='node name')
...
def load_model_at_init():
    """Load the H-optimus-0 model at initialization time. Unlike MUSK (vendored
    safetensors), H-optimus-0 is pulled from HuggingFace by timm — so there is
    no local weight file to locate. The repo can be overridden via the
    HOPTIMUS_MODEL_PATH env var (default the gated bioptimus/H-optimus-0)."""
    global MUSK_MODEL, NODE_NAME
    if MUSK_MODEL is not None:
        print(f"[{NODE_NAME}] H-optimus-0 model already loaded => skip")
        return

    try:
        model_path = os.environ.get("HOPTIMUS_MODEL_PATH", "hf-hub:bioptimus/H-optimus-0")
        device = "cuda" if torch.cuda.is_available() else "cpu"
        print(f"[{NODE_NAME}] Loading H-optimus-0 from {model_path}, device={device}")
        print(f"[{NODE_NAME}] (gated model — needs a HF token with access granted)")

        MUSK_MODEL = MUSK(model_path=model_path)
        print(f"[{NODE_NAME}] H-optimus-0 model loaded successfully")
...
def read_node(data: Dict[str, Any]):
    ...
    NODE_NAME = data.get("node_name", "HOptimusEmbedding")
