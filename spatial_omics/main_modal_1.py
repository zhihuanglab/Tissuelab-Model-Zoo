...
def run_cell2sentence(adata, user_query: str):
    ...
    # --- 2. Manually Generate Human-Readable Gene Sentences ---
    print("Generating human-readable gene sentences from AnnData...")
    gene_names = adata.var_names.tolist()
    ranked_genes_sentences = []

    # Ensure expression data is in a format that can be iterated (e.g., numpy array)
    expression_matrix = adata.X
    if hasattr(expression_matrix, "toarray"):  # Handle sparse matrices
        expression_matrix = expression_matrix.toarray()

    for spot_expression in expression_matrix:
        # Pair genes with their expression values
        sorted_genes = sorted(
            zip(gene_names, spot_expression), key=lambda x: x[1], reverse=True
        )
        # Create a sentence of gene names, ranked by expression
        ranked_gene_list = [
            gene for gene, expr in sorted_genes if expr > 0
        ]  # Only include expressed genes
        ranked_genes_sentences.append(" ".join(ranked_gene_list))

    # --- VISIBILITY STEP: Print the first few sentences for debugging ---
    print(f"✅ Generated {len(ranked_genes_sentences)} human-readable gene sentences.")
    print("👀 Example sentences (first 3):")
    for i, sentence in enumerate(ranked_genes_sentences[:3]):
        print(f"  - Spot {i+1}: {sentence[:200]}...")

    # Join with newlines for better model context and truncate to a safe length
    tissue_context = "\n".join(ranked_genes_sentences)
    max_context_length = 15000  # Approx 4000 words, a safe limit
    if len(tissue_context) > max_context_length:
        print(f"⚠️  Context length ({len(tissue_context)}) exceeds limit, truncating...")
        tissue_context = tissue_context[:max_context_length]

    # --- 3. Format the Prompt ---
    prompt = f"""You are an expert biologist analyzing spatial transcriptomics data from a tissue sample.
Below is a summary of the most highly expressed genes across different spots in the tissue. Each line represents one spot, with genes ranked by expression.
...
[YOUR ANALYSIS]
"""
    print("\n📝 Final prompt being sent to the model (truncated for preview):")
    print("-----------------------------------------")
    print(prompt[:1000] + "...")
    print("-----------------------------------------")
