...
class VisiumHDClusteringPipeline:
    ...
    def validate_config(self):
        ...
        required_keys = [
            'base_dir', 'bins_dir', 'segmentation_h5',
    ...
    def step1_load_binned_data(self):
        ...
        Loads the gene expression counts and spatial coordinates from the
        binned output directory generated by Space Ranger.
        """
        print("\n" + "="*80)
        print("STEP 1: Loading VisiumHD binned data")
        print("="*80)
        
        bins_dir = Path(self.config['bins_dir'])
        counts_h5 = bins_dir / "filtered_feature_bc_matrix.h5"
        spatial_pq = bins_dir / "spatial" / "tissue_positions.parquet"
        
        if not counts_h5.exists():
            raise FileNotFoundError(f"Counts file not found: {counts_h5}")
        if not spatial_pq.exists():
            raise FileNotFoundError(f"Spatial file not found: {spatial_pq}")
    ...
    def step2_load_segmentation(self):
        ...
        Reads nucleus contours and centroids from the segmentation H5 file.
        ...
        seg_h5 = Path(self.config['segmentation_h5'])
        if not seg_h5.exists():
            raise FileNotFoundError(f"Segmentation file not found: {seg_h5}")
        
        print(f"Reading from: {seg_h5}")
        with h5py.File(str(seg_h5), 'r') as f:
    ...
    def step8_save_results(self, adata):
        ...
        # Save AnnData
        h5ad_path = output_dir / f"clustering_results_k{n_clusters}.h5ad"
        print(f"Saving AnnData to: {h5ad_path}")
        adata.write_h5ad(str(h5ad_path), compression="gzip")
        
        # Save cluster assignments
        csv_path = output_dir / f"cluster_assignments_k{n_clusters}.csv"
        ...
        markers_path = output_dir / f"top_markers_k{n_clusters}.txt"
...
Example usage:
  python visiumhd_clustering_pipeline.py \\
    ...
    --bins-dir /path/to/binned_outputs/square_002um \\
    --segmentation-h5 /path/to/segmentation.h5 \\
    --n-clusters 9 \\
    --output-dir /path/to/output

This will process the VisiumHD data and generate clustering results with 9 domains.
        """
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Base directory containing VisiumHD data'
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Directory containing binned outputs (e.g., square_002um)'
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Path to segmentation H5 file containing nucleus contours and centroids'
    )
    ...
    config = {
        ...
        'segmentation_h5': args.segmentation_h5,
