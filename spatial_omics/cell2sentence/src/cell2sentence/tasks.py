...
from cell2sentence.prompt_formatter import C2SPromptFormatter, get_cell_sentence_str
...
def predict_cell_types_of_data(
    ...
    **kwargs
    ...
    # The original implementation called prompt_formatter.format_hf_ds(hf_ds_dict), which
    # fails for inference as it expects a 'cell_type' label for the response.
    # We will manually format only the input prompt here, which is all we need for prediction.

    # Predict cell types using trained C2S models
    print(f"Predicting cell types for {hf_ds_dict.num_rows} cells using CSModel...")
    predicted_cell_types = []
    for sample_idx in tqdm(range(hf_ds_dict.num_rows)):
        # Prepare inputs
        sample = hf_ds_dict[sample_idx]
        
        # Manually format the input prompt string
        single_cell_sentence_str, num_genes_str = get_cell_sentence_str(sample, num_genes=prompt_formatter.top_k_genes)
        sample["cell_sentence"] = single_cell_sentence_str
        sample["num_genes"] = num_genes_str
        model_input_keys, _ = prompt_formatter.get_keys_for_task()
        model_input_str_template = random.choice(prompt_formatter.prompts_dict["model_input"])
        model_input_prompt_str = model_input_str_template.format(**{key: sample[key] for key in model_input_keys})
