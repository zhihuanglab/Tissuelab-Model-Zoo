...
def hd_2D_stack(result, reference, pixelspacing=None, connectivity=1):
    '''
    binary object hd
    :param result: N*H*W
    :param reference: N*H*W
    ...
    zdim = result.shape[0]
    for i in range(zdim):
        if (np.sum(result[i, :, :]) > 0 and np.sum(reference[i, :, :]) > 0):
            hausdorff_distance = hd(
                result[i], reference[i], voxelspacing=pixelspacing, connectivity=connectivity)
            c = c + 1
            mean = mean + hausdorff_distance
        else:
            if (np.sum(result[i, :, :]) == 0 and np.sum(reference[i, :, :]) == 0):
                c = c
            else:
...
def volumesimilarity(result, reference):
    """
    ...
    return 2*(vol1 - vol2) / float(vol2+vol1)
...
def volume_change_correlation(results, references):
    ...
    references_volumes = numpy.asarray(
        [numpy.count_nonzero(r) for r in references])

    results_volumes_changes = results_volumes[1:] - results_volumes[:-1]
    references_volumes_changes = references_volumes[1:] - \
        references_volumes[:-1]
...
def obj_asd(result, reference, voxelspacing=None, connectivity=1):
    ...
    for lid2, lid1 in mapping.iteritems():
        ...
        sds.extend(__surface_distances(
            object1, object2, voxelspacing, connectivity))
    asd = numpy.mean(sds)
    return asd
