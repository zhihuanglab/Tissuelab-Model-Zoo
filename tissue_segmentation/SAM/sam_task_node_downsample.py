...
def read_rgb(image_path):
    """
    Read and process RGB image with automatic size adjustment
    """
    ...
    h, w = image.shape[:2]
    target_w, target_h = calculate_target_size(w, h, target_size=5000)
    
    print(f"[SAM] Original size: {w}x{h}, Target size: {target_w}x{target_h}")
    
    # Resize image
    out_img = np.zeros((target_h, target_w, 3), dtype=np.uint8)
    for i in range(3):
        out_img[:,:,i] = transform.resize(
            image[:,:,i], (target_h, target_w),
...
def read_wsi_to_patches(wsi_path, bbox, patch_size=1024, overlap=100):
    """
    Read WSI into patches with automatic level selection
    """
    import tiffslide
    import time
    ...
    # Find best level automatically
    best_level, (level_width, level_height) = find_best_wsi_level(svs, target_size=5000)
    downsample_factor = svs.level_downsamples[best_level]
    
    print(f"[SAM] WSI dimensions at level {best_level}: {level_width}x{level_height}")
    print(f"[SAM] Downsample factor: {downsample_factor}")
    
    # If no bbox provided, use full image at this level
    if bbox is None:
        bbox = [0, 0, level_width * downsample_factor, level_height * downsample_factor]
    ...
    print(f"[SAM] Processing WSI at level {best_level}")
    print(f"[SAM] Scaled size: {scaled_width}x{scaled_height}")
    print(f"[SAM] Grid: {n_rows}x{n_cols} patches (overlap={overlap}px)")
    ...
    end_time = time.time()
    print(f"[SAM] Processing time: {end_time - start_time:.2f} seconds")
...
def read_node(data: Dict[str, Any]):
    ...
    if "path" in user_data_dict:
        ...
        if path_str.lower().endswith(".svs") or path_str.lower().endswith(".tif"):
            USE_WSI = True
            if not BBOX:
                BBOX = None  # Let read_wsi_to_patches handle default bbox
...
def calculate_target_size(width, height, target_size=5000):
    """
...
def find_best_wsi_level(svs, target_size=5000):
    """
    Find the best level in WSI that gives dimensions close to target_size
    """
    dimensions = svs.level_dimensions
    best_level = 0
    min_diff = float('inf')
    
    for level, (width, height) in enumerate(dimensions):
        max_dim = max(width, height)
        diff = abs(max_dim - target_size)
        if diff < min_diff:
            min_diff = diff
            best_level = level
            
    return best_level, dimensions[best_level]
