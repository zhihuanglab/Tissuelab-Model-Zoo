...
# ====================== 3) Node Class ======================
class BiomedParseNode(TaskNode):
    """
   ...
   Notes:
        ...
        with safe_h5_open(self.h5_path, "r") as hf:
            ...
            if node_user_data in hf:
                ...
                for k in user_group.keys():
                    ...
                    if k == "image_path": # not wsi, jpg or png
                        self.image_path = val_json
                        # this is for test
                        self.image_path = "/Users/wangsheng/Library/Mobile Documents/com~apple~CloudDocs/postdoc/ZhiLab/TissueLab-AI-Service/toolbox/tissue_segmentation/BiomedParse/examples/Part_1_516_pathology_breast.png"
                        self.image = read_rgb(self.image_path)
    ...
    def execute(self):
       ...
       Finally return a dict as node's external execution result (for logs/viewing only).
        ...
        pred_mask = interactive_infer_image(self.model, Image.fromarray(self.image), [self.segmentation_prompt])
        pred_mask = pred_mask[0]

        polygons = mask_to_polygons(pred_mask)
        # result_value = f"Hello, I'm {self.name}. My param={self.some_param}."
        result_value = polygons
        # =======================================================
...
def main():
    ...
    node = BiomedParseNode(name=args.name, port=args.port)
    app = create_node_server(BiomedParseNode, node.name, node.port)
